...
# 进行查看分析.
# 2) (?<!exp) 叫零宽度负回顾后发断言, 断言此位置的后面不能匹配表达式 exp. 比如 (?<![a-z])\d{7} 匹配
# 前面不是小写字母的七位数字. 还有一个复杂的例子: (?<=<(\w+)>).*(?=<\/\1>), 用于匹配不包含属性的简单
# HTML 标记内的内容. 该表达式可以从 <div> python 爬虫 </div>中提取出 "python 爬虫", 这在 Python 爬虫
# 开发中常用到. 大家可以思考一下是如何提取出包含属性的 HTML 标记内的内容.

# 注释:
# 正则表达式可以包含注释进行解释说明, 通过语法 (?#comment) 来实现, 例如 \b\w+(?#字符串)\b.
# 要包含注释的话, 最好是启用 "忽略模式里的空白符" 选项, 这样在编写表达式时能任意地添加空格、
# Tab、换行, 而实际使用时这些都将被忽略.

# 贪婪与懒惰:
# 当正则表达式中包含能接受重复的限定符时, 通常的行为是(在使整个表达式能得到匹配的前提下)匹配尽可能多的
# 字符, 这就是贪婪模式. 以表达式 a\w+b 为例, 如果搜索 a12b34b, 会尽可能匹配更多的个数, 最后就会匹配整个
# a12b34b, 而不是 a12b. 但是如果相匹配出 a12b 怎么办呢? 这时候就需要使用懒惰模式, 尽可能匹配个数较少的
# 情况， 因此需要将上面的 a\w+b 表达式改为 a\w+?b, 使用 "?" 来启用懒惰模式.
# *?        重复任意次, 但尽可能少重复.
# +?        重复1次或更多次, 但尽可能少重复.
# ??        重复0次或1次, 但尽可能少重复.
# (n,m)?    重复n到m次, 但尽可能少重复.
...
# 处理选项:
# 一般正则表达式的实现库都提供了用来改变正则表达式处理选项的方式.
# 忽略大小写 匹配时不区分大小写.
# 多行模式   更改 "^" 和 "$" 的含义, 使它们分别在任意一行的行首和行尾匹配, 而不仅仅在整个字符串的开头和
#            结尾匹配.(在此模式下, "$" 的精确含义是: 匹配 "\n" 之前的位置以及字符串结束前的位置.)
# 单行模式   更改 "." 的含义, 使它与每一个字符匹配(包括换行符 "\n")
# 忽略空白   忽略表达式中的非转义空白并启用由 "#" 标记的注释.
# 显示捕获   仅捕获已被显示命名的组.

# 正则表达式中海油平衡组/递归匹配的概念, 对于初学者来说, 一般用不了这么复杂, 此处不进行讲解.